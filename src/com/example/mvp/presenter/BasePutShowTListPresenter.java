...
public class BasePutShowTListPresenter<T,V> {
	private BaseModel<T> mModel;
	private BasePutTListView<V> mPutView;
	...
	private BaseResponseListBean<V> mPutRsp = new BaseResponseListBean<V>();
	
	public BasePutShowTListPresenter(BasePutTListView<V> mPutView){
...
//	  			Type type = new TypeToken<BaseResponseBean<V>>(){}.getType();  type由所在的activity进行传递
	...
	private void showPut(){
		mPutView.putVList(rp, mPutRsp.getData(), mPutRsp.getStatus(), mPutRsp.getMsg());
	}
}
